<template>
 ...
 <div>
  ...
  <router-link  class="nav-link salesorder-block-panel"  v-for="(salesorder, index) in salesorders"  active-class="is-active" :to="'/salesorders/'+salesorder.id+'/view'" > 
  ...
  <div class="orderdetails">
      <div class="label is-field-title"> {{salesorder.sonumber}}  </div>
      <div class="label"> {{salesorder.customer.firstname}} {{salesorder.customer.lastname}} </div>
  </div>

  <div class="orderstatus">
     <div class="open"> {{salesorder.status}} </div>
     <div>{{salesorder.total}} </div>
  </div>
 ...
 export default {
  ...
  methods:{
      onTextChange: function(){
          ...
          this.salesorders=this.purchaseorders2.filter(item=>{
